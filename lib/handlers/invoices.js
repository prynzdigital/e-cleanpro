...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    const body = req.body || {};
    const { clientId, contractId, jobId, amount, dueDate, status, paymentMethod, paidDate, notes } = body;
    ...
    try {
      await ensureSchema();
      const rows = await sql`
        INSERT INTO invoices (client_id, contract_id, job_id, amount, due_date, status, payment_method, paid_date, notes)
        VALUES (

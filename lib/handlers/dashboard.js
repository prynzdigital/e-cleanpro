...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const [
      ...
      sql`SELECT COUNT(*)::int AS n, COALESCE(SUM(amount), 0)::float AS total FROM invoices WHERE status IN ('Unpaid', 'Overdue')`,
      sql`SELECT COUNT(*)::int AS n FROM reviews WHERE type = 'Complaint' AND status != 'Resolved'`,
      sql`SELECT ROUND(AVG(rating)::numeric, 1)::float AS avg, COUNT(*)::int AS n FROM reviews WHERE type = 'Review' AND rating IS NOT NULL`,
      sql`
        SELECT r.type, r.rating, r.status, r.created_at, c.name AS client_name
    ...
    const activity = [
      ...
      ...recentReviews.map((r) => ({
        type: r.type === 'Complaint' ? 'complaint' : 'review',
        label: r.type === 'Complaint' ? 'New Complaint' : 'New Review',
        detail: (r.client_name || 'Anonymous') + (r.rating ? ` — ${r.rating}★` : ''),
        at: r.created_at,
      })),
    ]
    ...
    res.status(200).json({
      ...
      unpaidInvoicesCount: unpaidInvoices[0].n,
      unpaidInvoicesTotal: unpaidInvoices[0].total,
      openComplaintsCount: openComplaints[0].n,
      avgRating: reviewStats[0].avg,
      totalReviewsCount: reviewStats[0].n,
    });
  } catch (err) {

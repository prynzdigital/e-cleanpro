...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      `;
      await logAudit(req, { action: 'create', resource: 'job-postings', resourceId: rows[0].id });
  ...
  if (req.method === 'PUT') {
    ...
    try {
      ...
      await logAudit(req, { action: 'update', resource: 'job-postings', resourceId: id });
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      ...
      await logAudit(req, { action: 'delete', resource: 'job-postings', resourceId: id });

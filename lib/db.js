...
async function ensureSchema() {
  ...
  schemaReady = (async () => {
    ...
    `;
    // Explicit ADD COLUMN (not a fresh CREATE TABLE field) since reviews
    // already existed in production before this column was added.
    await sql`ALTER TABLE reviews ADD COLUMN IF NOT EXISTS featured BOOLEAN NOT NULL DEFAULT false;`;
    // Public site submissions aren't tied to a client record, so capture
    // whatever contact info the submitter gave directly on the row.
    await sql`ALTER TABLE reviews ADD COLUMN IF NOT EXISTS submitted_by_name TEXT;`;
    await sql`ALTER TABLE reviews ADD COLUMN IF NOT EXISTS submitted_by_email TEXT;`;
    await sql`ALTER TABLE reviews ADD COLUMN IF NOT EXISTS submitted_by_phone TEXT;`;
    ...
    // Email is needed for self-service password reset; token columns back
    // that flow. All nullable since accounts created before this shipped
    // won't have an email on file until an Owner adds one.
    await sql`ALTER TABLE admin_users ADD COLUMN IF NOT EXISTS email TEXT;`;
    await sql`ALTER TABLE admin_users ADD COLUMN IF NOT EXISTS reset_token TEXT;`;
    await sql`ALTER TABLE admin_users ADD COLUMN IF NOT EXISTS reset_token_expires TIMESTAMPTZ;`;
    // Postgres allows multiple NULLs under a UNIQUE index, so this is safe
    // even though existing accounts don't have an email on file yet.
    await sql`CREATE UNIQUE INDEX IF NOT EXISTS admin_users_email_unique ON admin_users (lower(email)) WHERE email IS NOT NULL;`;

    // Records who did what to which record, for accountability across the
    // whole admin app. user_id/username/role are a snapshot at the time of
    // the action (no FK) so history survives a user being deleted later.
    await sql`
    ...
    `;
    // Explicit ADD COLUMN (not fresh CREATE TABLE fields) since
    // job_applications already existed in production before these were
    // added. race/veteran_status/military_status are voluntary EEO
    // self-identification fields -- never required, kept separate from
    // the hiring decision.
    await sql`ALTER TABLE job_applications ADD COLUMN IF NOT EXISTS address TEXT;`;
    await sql`ALTER TABLE job_applications ADD COLUMN IF NOT EXISTS zip_code TEXT;`;
    await sql`ALTER TABLE job_applications ADD COLUMN IF NOT EXISTS race TEXT;`;
    await sql`ALTER TABLE job_applications ADD COLUMN IF NOT EXISTS veteran_status TEXT;`;
    await sql`ALTER TABLE job_applications ADD COLUMN IF NOT EXISTS military_status TEXT;`;
  })();

...
async function fetchReviewById(id) {
  const rows = await sql`
    SELECT
      ...
      r.status, r.resolution, r.featured, r.created_at,
      r.submitted_by_name, r.submitted_by_email, r.submitted_by_phone,
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    const body = req.body || {};
    const { clientId, jobId, type, rating, source, comment, status, resolution, featured } = body;
    ...
    try {
      await ensureSchema();
      const rows = await sql`
        INSERT INTO reviews (client_id, job_id, type, rating, source, comment, status, resolution, featured)
        VALUES (
          ${toNumberOrNull(clientId)}, ${toNumberOrNull(jobId)}, ${normalizeType(type)}, ${toIntOrNull(rating)},
          ${source || null}, ${comment.trim()}, ${normalizeStatus(status)}, ${resolution || null}, ${!!featured}
        )
        RETURNING id
      `;
      const full = await fetchReviewById(rows[0].id);
      await logAudit(req, { action: 'create', resource: 'reviews', resourceId: rows[0].id });
  ...
  if (req.method === 'PUT') {
    ...
    try {
      ...
      await logAudit(req, { action: 'update', resource: 'reviews', resourceId: id });
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      ...
      await logAudit(req, { action: 'delete', resource: 'reviews', resourceId: id });

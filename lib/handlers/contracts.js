...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      await logAudit(req, { action: 'create', resource: 'contracts', resourceId: rows[0].id });
  ...
  if (req.method === 'PUT') {
    ...
    try {
      ...
      await logAudit(req, { action: 'update', resource: 'contracts', resourceId: id });
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      ...
      await logAudit(req, { action: 'delete', resource: 'contracts', resourceId: id });

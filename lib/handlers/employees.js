...
const STATUS_VALUES = ['Active', 'Inactive', 'Waiting List'];
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      `;
      await logAudit(req, { action: 'create', resource: 'employees', resourceId: rows[0].id });
  ...
  if (req.method === 'PUT') {
    ...
    try {
      ...
      await logAudit(req, { action: 'update', resource: 'employees', resourceId: id });
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      ...
      await logAudit(req, { action: 'delete', resource: 'employees', resourceId: id });

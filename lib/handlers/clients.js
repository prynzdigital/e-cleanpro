...
module.exports = async (req, res) => {
    ...
    try {
      ...
      const rows = await sql`
        ...
        RETURNING id, name, company, phone, email, address, service_type, square_footage,
                  cleaning_frequency, contract_amount, notes, status, source_lead_id, created_at
      `;
      await logAudit(req, { action: 'create', resource: 'clients', resourceId: rows[0].id });
  ...
  if (req.method === 'PUT') {
    ...
    try {
      ...
      await logAudit(req, { action: 'update', resource: 'clients', resourceId: id });
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      ...
      await logAudit(req, { action: 'delete', resource: 'clients', resourceId: id });
